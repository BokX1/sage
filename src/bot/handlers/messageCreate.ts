...
export async function handleMessageCreate(message: Message) {
    ...
    try {
        ...
        const result = await generateChatReply({
            ...
            userText: text,
            replyToBotText: isReplyToBot && message.reference ? (await message.fetchReference()).content : null
        });
...
export function registerMessageCreateHandler() {
    ...
    logger.info({ count: client.listenerCount(Events.MessageCreate) }, 'MessageCreate handler registered');
}
