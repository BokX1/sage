...
export async function generateChatReply(params: {
    ...
    if (profileSummary) {
        messages.push({ role: 'system', content: `Personalization memory (may be incomplete): ${profileSummary}` });
    }
    ...
    try {
        ...
        const response = await client.chat({
            ...
            tools: isGeminiNative ? [{ googleSearch: {} }] : undefined,
            toolChoice: isGeminiNative ? 'auto' : undefined,
            temperature: 0.7,
        });
    ...
    const BANNED_PHRASES = [
        'google search', 'internet search', 'i searched', 'i found online',
        'browsing', 'tool', 'function call', 'search result', 'i looked up',
        'querying', 'searching', 'according to my search', 'my internal tools'
    ];
    ...
    if (hasBanned) {
        ...
        try {
            const rewriteResponse = await client.chat({
                messages: [
                    { role: 'system', content: 'Rewrite the response to remove any mention of searching/browsing/tools/process. Keep meaning. Keep References if present.' },
            ...
            console.error('Guardrail rewrite failed', e);
        }
    }
    ...
    updateProfileSummary({
        ...
        if (newSummary && newSummary !== profileSummary) {
            upsertUserProfile(userId, newSummary).catch(err => console.error('Failed to save profile', err));
        }
    });
