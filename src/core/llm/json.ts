...
`.trim();
    ...
    try {
        ...
        console.warn('[callWithSchema] Initial call failed or invalid JSON:', error);
        // 2. Repair Attempt
        return repairJsonOnce(client, schema, messages, error instanceof Error ? error.message : String(error));
    }
}
...
async function repairJsonOnce<T>(
    ...
    const repairMessages: LLMChatMessage[] = [
        ...originalMessages,
        { role: 'user', content: `Your previous response was invalid JSON. Error: ${errorMsg}. Please output ONLY valid JSON matching the schema.` }
    ];
    ...
    try {
        ...
        console.error('[callWithSchema] Repair failed:', finalError);
        return null;
    }
}
